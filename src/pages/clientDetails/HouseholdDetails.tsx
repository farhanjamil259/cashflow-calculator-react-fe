...
const HouseholdDetails = () => {
  const inputs: IInputs = useSelector((state: RootStateOrAny) => state.inputsReducer)[0];
  ...
  useEffect(() => {
    ...
    totalIncomeTemp +=
      ...
      cy <= inputs.household_income.rental_income.details[0].end_year
        ...
        data: [
          {
            ...
            y: inputs.assets.properties.reduce((sum: any, p: any) => {
              if (inputs.current_year <= p.end_year && inputs.current_year >= p.start_year) {
          ...
          {
            name: "Pension Plan",
            y: inputs.assets.non_employment_defined_contribution_pension_plans.reduce((sum: any, p: any) => {
              if (
          ...
          },
          {
            ...
            y:
              inputs.assets.savings_and_investments.individual_savings_account.reduce((sum: any, p: any) => {
                if (
                  inputs.current_year <= p.contribution_end_year &&
                  inputs.current_year >= p.contribution_start_year
                ) {
              ...
              inputs.assets.savings_and_investments.general_investment_account.reduce((sum: any, p: any) => {
                if (
                  inputs.current_year <= p.contribution_end_year &&
                  inputs.current_year >= p.contribution_start_year
                ) {
  ...
  const incomeChartOptions: Options = {
    ...
    colors: ["#388e3c", "#43a047", "#4caf50", "#66bb6a", "#81c784", "#a5d6a7", "#c8e6c9"],
    series: [
      {
        name: "Income",
        colorByPoint: true,
        ...
        data: [
          {
            ...
            y: inputs.household_income.employment_income.reduce((sum: any, p: any) => {
              if (inputs.current_year <= p.end_year && inputs.current_year >= p.start_year) {
          ...
          },
          {
            ...
            y: inputs.household_income.self_employment_income.reduce((sum: any, p: any) => {
              if (inputs.current_year <= p.end_year && inputs.current_year >= p.start_year) {
                return sum + p.gross_anual_amount;
              } else {
                return sum;
          ...
          },
          {
            name: "Rental Income",
            y:
              inputs.household_income.rental_income.details[0].start_year >= inputs.current_year &&
              inputs.household_income.rental_income.details[0].end_year <= inputs.current_year
                ? inputs.household_income.rental_income.joint_annual_rental_income
                : 0,
          },
          {
            name: "Dividend Income",
            y: inputs.household_income.dividend_income.reduce((sum: any, p: any) => {
              if (inputs.current_year <= p.end_year && inputs.current_year >= p.start_year) {
          ...
          },
          {
            name: "Pension Income",
            y:
              inputs.household_income.pension_income.state_pension.reduce((a: any, b: any) => {
                if (inputs.current_year <= b.end_year && inputs.current_year >= b.start_year) {
                  return a + b.amount_to_drawn_down;
                } else {
                  return a;
  ...
  const expensesChartOptions: Options = {
    ...
    colors: ["#c62828", "#d32f2f", "#e53935", "#f44336", "#ef5350", "#e57373", "#ef9a9a", "#ffcdd2"],
    ...
    series: [
      {
        ...
        data: [
          {
            name: "Housing",
            y: inputs.household_expenses.housing.details.reduce((a: any, b: any) => {
              if (inputs.current_year <= b.end_year && inputs.current_year >= b.start_year) {
          ...
          {
            name: "Consumables",
            y: inputs.household_expenses.consumables.details.reduce((a: any, b: any) => {
              if (inputs.current_year <= b.end_year && inputs.current_year >= b.start_year) {
                return a + b.annual_expense;
              } else {
                return a;
          ...
          {
            name: "Travel",
            y: inputs.household_expenses.travel.details.reduce((a: any, b: any) => {
              if (inputs.current_year <= b.end_year && inputs.current_year >= b.start_year) {
                return a + b.annual_expense;
              } else {
                return a;
          ...
          {
            name: "Shopping",
            y: inputs.household_expenses.shopping.details.reduce((a: any, b: any) => {
              if (inputs.current_year <= b.end_year && inputs.current_year >= b.start_year) {
                return a + b.annual_expense;
              } else {
                return a;
          ...
          {
            name: "Entertainment",
            y: inputs.household_expenses.entertainment.details.reduce((a: any, b: any) => {
              if (inputs.current_year <= b.end_year && inputs.current_year >= b.start_year) {
                return a + b.annual_expense;
              } else {
                return a;
          ...
          {
            name: "Holiday",
            y: inputs.household_expenses.holiday.details.reduce((a: any, b: any) => {
              if (inputs.current_year <= b.end_year && inputs.current_year >= b.start_year) {
                return a + b.annual_expense;
              } else {
                return a;
          ...
          },
          {
            ...
            y: inputs.household_expenses.one_off_expenses.reduce((a: any, b: any) => {
              if (inputs.current_year <= b.end_year && inputs.current_year >= b.start_year) {
                  ...
                  <Col span={24}>
                    <Card bordered={false}>
                      <Row justify="space-around">
                        <Col span={6}>
                          <Row justify={"start"}>
                            <Text style={{ width: "100%", textAlign: "left" }} strong>
                              Net Worth
                            </Text>
                            ...
                            <Col span={12}>
                              <Row justify="space-around">
                                ...
                                <Text className="liability-amount">
                                  -{pound}
                                  {numberFormat(totalLiabilities, 0, ".", ",")}
                                </Text>
                              </Row>
                            </Col>
                          </Row>
