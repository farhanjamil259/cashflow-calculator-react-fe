      ...
      >
        <Form form={form} labelAlign="left" labelCol={{ span: 8 }} wrapperCol={{ span: 16 }}>
          <Form.Item label="Expenses Name">
            <Text>{inputs.household_expenses.housing.details[activeItemIndex].name}</Text>
          </Form.Item>
          <Form.Item label="Share of Rental Income">
            <Text>
              <CurrencyFormat
                value={inputs.household_expenses.housing.details[activeItemIndex].annual_expense}
                displayType={"text"}
          ...
          <Form.Item label="Inflation" rules={[{ required: true, message: "This is required" }]}>
            <Text>
              <CurrencyFormat
                value={inputs.household_expenses.housing.details[activeItemIndex].inflation * 100}
                displayType={"text"}
                decimalScale={2}
                suffix={"%"}
              />
          ...
          <Form.Item label="Start year">
            <Text>{inputs.household_expenses.housing.details[activeItemIndex].start_year}</Text>
          </Form.Item>
          <Form.Item label="End year">
            <Text>{inputs.household_expenses.housing.details[activeItemIndex].end_year}</Text>
          </Form.Item>
          <Form.Item label="Share of Rental Income">
            <Text>
              <CurrencyFormat
                value={inputs.household_expenses.housing.details[activeItemIndex].rate_after_retirement * 100}
                displayType={"text"}
                decimalScale={2}
                suffix={"%"}
              />
