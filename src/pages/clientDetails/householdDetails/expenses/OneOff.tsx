...
const OneOff = () => {
    ...
    const [expensesDetails, setExpensesDetails] = useState(
        JSON.parse(JSON.stringify(inputs.household_expenses.one_off_expenses))
    );
                ...
                <Form form={form} labelAlign="left" labelCol={{span: 10}} wrapperCol={{span: 14}}>
                    <Form.Item label="Expense Name">
                        {editMode ? (
                            ...
                            <Text>
                                {
                                    inputs.household_expenses.one_off_expenses[activeItemIndex]
                                        .name
                                }
                            </Text>
                        )}
                    ...
                    <Form.Item label="Annual Payment in Todays Terms">
                        {editMode ? (
                            <InputNumber
                                value={expensesDetails[activeItemIndex].annual_payment_in_todays_terms}
                                style={{width: "100%"}}
                                onChange={(e) => {
                                    const clone = JSON.parse(JSON.stringify(expensesDetails));
                                    clone[activeItemIndex].annual_payment_in_todays_terms = +e!;
                                    setExpensesDetails(clone);
                                }}
                            />
                        ) : (
