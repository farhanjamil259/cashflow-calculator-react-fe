      ...
      >
        <Form form={form} labelAlign="left" labelCol={{ span: 8 }} wrapperCol={{ span: 16 }}>
          <Form.Item label="Expense Name">
            <Text>{inputs.household_expenses.travel.details[activeItemIndex].name}</Text>
          </Form.Item>
          <Form.Item label="Annual Expense">
            <Text>
              <CurrencyFormat
                value={inputs.household_expenses.travel.details[activeItemIndex].annual_expense}
                displayType={"text"}
          ...
          <Form.Item label="Inflation" rules={[{ required: true, message: "This is required" }]}>
            <Text>
              <CurrencyFormat
                value={inputs.household_expenses.travel.details[activeItemIndex].inflation * 100}
                displayType={"text"}
                decimalScale={2}
                suffix={"%"}
              />
          ...
          <Form.Item label="Start year">
            <Text>{inputs.household_expenses.travel.details[activeItemIndex].start_year}</Text>
          </Form.Item>
          <Form.Item label="End year">
            <Text>{inputs.household_expenses.travel.details[activeItemIndex].end_year}</Text>
          </Form.Item>
          <Form.Item label="% After Retirement">
            <Text>
              <CurrencyFormat
                value={inputs.household_expenses.travel.details[activeItemIndex].rate_after_retirement * 100}
                displayType={"text"}
                decimalScale={2}
                suffix={"%"}
              />
